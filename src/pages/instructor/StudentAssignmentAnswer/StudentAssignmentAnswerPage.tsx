...
export default function StudentAssignmentAnswerPage() {
  ...
  const { data, isLoading, isError } = useGetAssignmentAnswersQuery(
    { student_id: parsedId },
    { skip: !parsedId }
  );
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center h-64 text-gray-500">
        ...
        <p>Loading assignment answers...</p>
      </div>
    );
  }
  ...
  if (isError || !data?.data?.length) {
    return (
      <div className="flex flex-col items-center justify-center h-64 space-y-3 text-center">
        <FileText className="w-12 h-12 text-gray-400" />
        <p className="text-gray-600 font-medium">
          No assignment answers found for this student.
        </p>
        <Link
          to="/instructor/enrollments"
          className="flex items-center text-brand-500 hover:text-brand-700 text-sm font-medium"
      ...
      <header className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b pb-4">
        <div>
          ...
          <p className="text-sm text-gray-500">
            Viewing submitted assignments by student ID:{" "}
            ...
            >
              ...
              {isOpen && (
                <div className="pt-2 border-t border-gray-100 dark:border-gray-700 text-sm space-y-1 text-gray-700 dark:text-gray-300">
                  <p>
                    <span className="font-semibold">Answer Description:</span>{" "}
                    {answer.description || "-"}
                  </p>
                  <p>
                  ...
                  <p>
                    ...
                    {answer.assignment?.expiration_date
                      ? new Date(
                          answer.assignment.expiration_date
                        ).toLocaleDateString()
                      : "-"}
